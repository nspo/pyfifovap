...
def extract_year_quotes(
    ...
    in_year = history[history.index.year == year]
    # a NaN close would survive every later check: max(0.0, nan) is 0.0, so the
    # security would silently end up with a Vorabpauschale of zero
    in_year = in_year[in_year["Close"].notna()]
    if in_year.empty:
    ...
    distributions = []
    if "Dividends" in in_year.columns:
        amounts = pd.to_numeric(in_year["Dividends"], errors="coerce")
        if amounts.isna().any():
